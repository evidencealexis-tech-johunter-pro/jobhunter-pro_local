...
export default function Settings() {
  ...
  const [savingMatching, setSavingMatching] =
    useState(false);
  ...
  async function loadAll() {
    try {
      const [
        ...
        base44.entities.ScrapeSource.list(
          "-created_date",
          50
        ),
      ]);
  ...
  async function handleSaveMatching() {
    ...
    try {
      ...
      toast.success(
        "Matching engine saved."
      );
    } catch {
      toast.error(
        "Failed to save matching."
      );
    } finally {
  ...
  const needsBaseUrl =
    selectedProviderInfo?.needs_base_url;
  ...
  async function handleSaveKey() {
    ...
    if (
      ...
      toast.error(
        "Custom providers need a Base URL and a model name."
      );
      return;
    }
  ...
  async function handleToggleSource(
    ...
    try {
      const updated =
        await base44.entities.ScrapeSource.update(
          source.id,
          {
  ...
  async function handleDeleteSource(
    ...
    try {
      await base44.entities.ScrapeSource.delete(
        source.id
      );
    ...
    <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Control your matching, sources, and AI provider
        </p>
      </div>
              ...
              <div className="flex items-center justify-between gap-2 text-sm text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg">
                <span className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="truncate">
                    Active:{" "}
                    {keyStatus.provider?.toUpperCase()}{" "}
                    ({keyStatus.model}) — ****
                    {keyStatus.key_suffix}
                  </span>
                </span>
        ...
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Jobs scoring above your threshold appear in your matches
            </p>
          </div>
        ...
        {[
          {
            ...
            desc: "How much skill matching matters",
          },
          {
            key: "semantic_weight",
            label: "Semantic similarity",
            desc: "Overall meaning match",
          },
          {
            key: "seniority_weight",
            label: "Seniority fit",
            desc: "Experience level alignment",
          },
          {
            key: "domain_weight",
            label: "Domain fit",
            desc: "Industry/background match",
          },
        ].map((weight) => (
          <div
            key={weight.key}
      ...
      <Card className="p-5 border-slate-200 shadow-sm">
        ...
        <p className="text-xs text-slate-500 mb-4">
          Website URLs to scan for job postings
        </p>
        ...
        {sources.length === 0 ? (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-slate-400">
              No sources yet. Add a career page URL above.
            </p>
          </div>
        ) : (
      ...
      <Card className="p-5 border-slate-200 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Get alerted when new matching jobs are found
            </p>
          </div>
        ...
        <SettingRow
          ...
          description="Message me on Telegram when new matches appear"
        >
          <Switch
            checked={
        ...
        <SettingRow
          ...
          description="Notify about interview invitations"
        >
          <Switch
            checked={
              settings?.interview_alerts ||
              false
            }
            onCheckedChange={(value) =>
              update(
